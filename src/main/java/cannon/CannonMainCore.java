    ...
    public CannonMainCore(String fileAddress, int torusLength, String compress){
        ...
        for (int i = 0; i < torusLength; i++) {
            for (int j = 0; j < torusLength; j++) {
                ...
                coreWorkers[i][j] = new CannonWorker(i, j, torusLength, coreDownChannels[i][j], coreUpChannels[i][j], hchannels[i][j], vchannels[i][j], hchannels[i][(j + 1) % torusLength], vchannels[(i + 1) % torusLength][j], cache, blockCount);
